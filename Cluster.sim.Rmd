...
mean degree (via partner number per timestep) and per-contact transmission rate in the 
model, and then vary the sample fraction, sample bias, and
sampling time after infection after the model has been run.
...
(These are initial parameters, if using the heterogeneous transmission option)

```{r}

heterogeneous_risk <- 1     # if 0, all rates are the same for all individuals

mean_partner_parameter <- 0.5  # parameters for geom distribution for mean number of (susceptible) partners per timestep
acts_per_day_parameter <- 2   # mean sex acts per day per partner 
lambda_parameter <- 0.003  # mean risk of transmission per sero-discordant contact

# Removal rate parameter
removal_rate_parameter <- 1/1000 # daily probability of being removed
```
